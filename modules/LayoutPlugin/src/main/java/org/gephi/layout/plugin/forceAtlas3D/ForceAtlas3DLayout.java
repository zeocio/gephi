...
import org.gephi.layout.plugin.forceAtlas3D.ForceFactory.AttractionForce;
import org.gephi.layout.plugin.forceAtlas3D.ForceFactory.RepulsionForce;
...
/**
 * ForceAtlas 2 3D Layout, manages each step of the computations.
 *
 * @author Mathieu Jacomy
 */
public class ForceAtlas3DLayout implements Layout {

    private GraphModel graphModel;
    private Graph graph;
    private final ForceAtlas3DBuilder layoutBuilder;
    ...
    public ForceAtlas3DLayout(ForceAtlas3DBuilder layoutBuilder) {
    ...
    public void initAlgo() {
        ...
        for (Node n : nodes) {
            if (n.getLayoutData() == null || !(n.getLayoutData() instanceof ForceAtlas3DLayoutData)) {
                ForceAtlas3DLayoutData nLayout = new ForceAtlas3DLayoutData();
                n.setLayoutData(nLayout);
            }
            ForceAtlas3DLayoutData nLayout = n.getLayoutData();
...
//        System.out.printf("XXX0: " + taskCount + " ");
        ...
        for (Future future : threads) {
            try {
                future.get();
//                System.out.printf("XXX01: " + future + ", ");
...
//        System.out.printf("XXX1: %f %f %f\n", speed, totalSwinging, totalEffectiveTraction);
...
//        System.out.printf("XXX2: %f %f %f %f\n", targetSpeed, speed, totalSwinging, totalEffectiveTraction);
        ...
        if (isAdjustSizes()) {
            ...
            for (Node n : nodes) {
                ...
                if (!n.isFixed()) {
                    ...
                    double swinging = nLayout.mass * Math.sqrt((nLayout.old_dx - nLayout.dx) * (nLayout.old_dx - nLayout.dx) + (nLayout.old_dy - nLayout.dy) * (nLayout.old_dy - nLayout.dy) + (nLayout.old_dz - nLayout.dz) * (nLayout.old_dz - nLayout.dz));
